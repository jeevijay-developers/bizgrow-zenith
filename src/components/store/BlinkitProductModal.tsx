...
const TrustBadge = ({ icon: Icon, title, subtitle }: { icon: any; title: string; subtitle: string }) => (
  <div className="flex flex-col items-center text-center p-3 bg-gray-50 rounded-xl flex-1">
    <Icon className="h-5 w-5 text-emerald-500 mb-1" />
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 sticky top-0 bg-white z-10">
        ...
        <h2 className="font-medium text-gray-800 text-sm truncate max-w-[50%]">
          {product.name}
        </h2>
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        ...
        <div className="px-4 py-4">
          {/* Delivery Badge */}
          <div className="inline-flex items-center gap-1.5 bg-emerald-50 px-2 py-1 rounded-full mb-3">
            <Clock className="h-3.5 w-3.5 text-emerald-600" />
            <span className="text-xs font-semibold text-emerald-700">13 MINS</span>
          </div>
                ...
                <div className="space-y-2">
                  {details.map((detail, i) => (
                    <div key={i} className="flex justify-between">
                      ...
                      <span className="text-gray-800 font-medium capitalize">{detail.value}</span>
                    </div>
                  ))}
                </div>
              </AccordionSection>
          ...
          {whatsappNumber && (
            <Button
              ...
              variant="outline"
              className="w-full mb-4 border-emerald-500 text-emerald-600 hover:bg-emerald-50"
            >
          ...
          {similarProducts.length > 0 && (
            <div className="mt-6">
              <h3 className="font-bold text-gray-900 mb-3">Similar Products</h3>
              <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4">
                {similarProducts.slice(0, 6).map((p) => (
                  ...
                  >
                    <div className="bg-gray-50 rounded-xl aspect-square mb-2 overflow-hidden">
                      {p.image_url ? (
                        <img
                          src={p.image_url}
                          alt={p.name}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 flex items-center justify-between z-20">
        ...
        {isOutOfStock ? (
          ...
          <div className="flex items-center gap-0 bg-emerald-500 rounded-lg overflow-hidden">
            <button
              onClick={() => onUpdateQuantity(-1)}
              className="p-3 hover:bg-emerald-600 text-white transition-colors"
            >
              <Minus className="h-4 w-4" />
            </button>
            <span className="px-4 text-white font-bold text-lg min-w-[40px] text-center">
              {quantity}
            </span>
            <button
              onClick={() => onUpdateQuantity(1)}
