...
const extractDetails = (description: string | null) => {
  ...
  return { details, cleanDescription: description.slice(0, 200) };
};
...
const TrustBadge = ({ icon: Icon, title, subtitle }: { icon: any; title: string; subtitle: string }) => (
  <div className="flex flex-col items-center text-center p-2 bg-muted rounded-lg flex-1">
    ...
    <div className="border-b border-border">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="pb-2 text-xs text-muted-foreground">{children}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border sticky top-0 bg-background z-10">
        ...
        <h2 className="font-medium text-foreground text-xs truncate max-w-[50%]">
          {product.name}
        </h2>
        ...
        <div className="px-3 py-3">
          ...
          <div className="border-t border-border">
            <AccordionSection title="Product Details" defaultOpen>
              <p className="text-muted-foreground text-xs leading-relaxed">
                {cleanDescription || "Quality product from trusted seller."}
              </p>
            </AccordionSection>
          ...
          {storeName && (
            <div className="flex items-center justify-between p-2 bg-muted rounded-lg mb-3">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-background rounded-md flex items-center justify-center border border-border">
                  <Store className="h-4 w-4 text-muted-foreground" />
                </div>
                <div>
                  <p className="font-medium text-foreground text-xs">{storeName}</p>
                  <p className="text-[10px] text-muted-foreground">View store</p>
                </div>
              </div>
          ...
          {whatsappNumber && (
            <Button
              ...
              variant="outline"
              size="sm"
              className="w-full mb-3 border-primary text-primary hover:bg-primary/10 text-xs h-8"
            >
          ...
          {similarProducts.length > 0 && (
            <div className="mt-4">
              <h3 className="font-semibold text-foreground text-sm mb-2">Similar Products</h3>
              <div className="flex gap-2 overflow-x-auto pb-2 -mx-3 px-3">
                {similarProducts.slice(0, 6).map((p) => (
                  ...
                  >
                    <div className="bg-muted rounded-lg aspect-square mb-1 overflow-hidden">
                      {p.image_url ? (
                        <img
                          src={p.image_url}
                          alt={p.name}
                    ...
                    <p className="text-[10px] font-bold text-foreground">₹{p.price}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border px-3 py-2 flex items-center justify-between z-20">
        ...
        {isOutOfStock ? (
          <Button disabled size="sm" className="bg-muted text-muted-foreground px-6 h-8">
            Out of Stock
          </Button>
        ) : quantity === 0 ? (
          <Button
            onClick={onAddToCart}
            size="sm"
            className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold px-6 h-8"
          >
            Add to cart
          </Button>
        ) : (
          <div className="flex items-center gap-0 bg-primary rounded-md overflow-hidden">
            <button
              onClick={() => onUpdateQuantity(-1)}
              className="p-2 hover:bg-primary/80 text-primary-foreground transition-colors"
            >
              <Minus className="h-3.5 w-3.5" />
            </button>
            <span className="px-3 text-primary-foreground font-bold text-sm min-w-[28px] text-center">
              {quantity}
            </span>
            <button
              onClick={() => onUpdateQuantity(1)}
